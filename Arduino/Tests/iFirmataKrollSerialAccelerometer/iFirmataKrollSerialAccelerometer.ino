...
int samplingInterval = 90;          // how often to report analog and servo data

unsigned long previousMillisBle = 0;
unsigned long bleInterval = 30;
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  case CAPABILITY_QUERY:
    ...
    iFirmata.bleSend(START_SYSEX);
    iFirmata.bleSend(CAPABILITY_RESPONSE);
    for (byte pin=0; pin < TOTAL_PINS; pin++) {
      if (IS_PIN_DIGITAL(pin)) {
        iFirmata.bleSend((byte)INPUT);
        iFirmata.bleSend(1);
        iFirmata.bleSend((byte)OUTPUT);
        iFirmata.bleSend(1);
      }
      if (IS_PIN_ANALOG(pin)) {
        iFirmata.bleSend(ANALOG);
        iFirmata.bleSend(10);
      }
      if (IS_PIN_PWM(pin)) {
        iFirmata.bleSend(PWM);
        iFirmata.bleSend(8);
      }
      if (IS_PIN_SERVO(pin)) {
        iFirmata.bleSend(SERVO);
        iFirmata.bleSend(14);
      }
      if (IS_PIN_I2C(pin)) {
        iFirmata.bleSend(I2C);
        iFirmata.bleSend(1);  // to do: determine appropriate value 
      }
  ...
  case PIN_STATE_QUERY:
    ...
    if (argc > 0) {
      ...
      iFirmata.bleSend(START_SYSEX);
      iFirmata.bleSend(PIN_STATE_RESPONSE);
      iFirmata.bleSend(pin);
  ...
  case ANALOG_MAPPING_QUERY:
    ...
    iFirmata.bleSend(START_SYSEX);
    iFirmata.bleSend(ANALOG_MAPPING_RESPONSE);
    for (byte pin=0; pin < TOTAL_PINS; pin++) {
      iFirmata.bleSend(IS_PIN_ANALOG(pin) ? PIN_TO_ANALOG(pin) : 127);
    }
    iFirmata.bleSend(END_SYSEX);
    break;
  }  
...
void setup() {
  ...
  iFirmata.attach(DIGITAL_MESSAGE, digitalWriteCallback);
  iFirmata.attach(REPORT_ANALOG, reportAnalogCallback);
  iFirmata.attach(REPORT_DIGITAL, reportDigitalCallback);
  iFirmata.attach(SET_PIN_MODE, setPinModeCallback);
    ...
    for(pin=0; pin<TOTAL_PINS; pin++) {
      if (IS_PIN_ANALOG(pin) && pinConfig[pin] == ANALOG) {
        ...
        if (analogInputsToReport & (1 << analogPin)) {
          iFirmata.sendAnalog(analogPin, analogRead(analogPin));
        }
      }
    }
