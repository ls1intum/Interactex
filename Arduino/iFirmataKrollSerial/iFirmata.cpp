...
void iFirmataClass::sendValueAsTwo7bitBytes(int value)
{
    bleSend(value & B01111111);
    bleSend(value >> 7 & B01111111);
}

void iFirmataClass::startSysex(void)
{
  bleSend(START_SYSEX);
}

void iFirmataClass::endSysex(void)
{
  bleSend(END_SYSEX);
}
...
iFirmataClass::iFirmataClass(Stream &s) : iFirmataSerial(s)
{
  sendBufferStart = 0;
  sendBufferCount = 0;
...
void iFirmataClass::begin(void)
{
  begin(19200);
}

/* begin method for overriding default serial bitrate */
void iFirmataClass::begin(long speed)
{
  Serial.begin(speed);
  iFirmataSerial = Serial;
  blinkVersion();
  printVersion();
  printFirmwareVersion();
}

void iFirmataClass::begin(Stream & s)
{
  iFirmataSerial = s;
  systemReset();
  printVersion();
...
void iFirmataClass::blinkVersion(void)
{    
...
void iFirmataClass::printFirmwareVersion(void) {
    byte i;
...
void iFirmataClass::setFirmwareNameAndVersion(const char *name, byte major, byte minor)
{
  const char *filename;
  char *extension;
...
int iFirmataClass::available(void)
{
  return iFirmataSerial.available();
}

void iFirmataClass::processSysexMessage(void)
{
...
void iFirmataClass::processInput(void)
{
    
  int inputData = iFirmataSerial.read(); // this is 'int' to handle -1 when no data
  int command;
...
void iFirmataClass::sendAnalog(byte pin, int value) 
{
...
void iFirmataClass::sendDigital(byte pin, int value) 
{
...
void iFirmataClass::sendDigitalPort(byte portNumber, int portData)
{    
...
void iFirmataClass::sendSysex(byte command, byte bytec, byte* bytev) 
{
  byte i;
  startSysex();
  bleSend(command);
...
void iFirmataClass::sendString(byte command, const char* string) 
{
...
void iFirmataClass::sendString(const char* string) 
{
...
// generic callbacks
void iFirmataClass::attach(byte command, callbackFunction newFunction)
{
  switch(command) {
...
void iFirmataClass::attach(byte command, systemResetCallbackFunction newFunction)
{
  switch(command) {
...
void iFirmataClass::attach(byte command, stringCallbackFunction newFunction)
{
  switch(command) {
...
void iFirmataClass::attach(byte command, sysexCallbackFunction newFunction)
{
  currentSysexCallback = newFunction;
}

void iFirmataClass::detach(byte command)
{
  switch(command) {
...
void iFirmataClass::systemReset(void)
{
  byte i;
...
void iFirmataClass::pin13strobe(int count, int onInterval, int offInterval) 
{
  byte i;
  pinMode(VERSION_BLINK_PIN, OUTPUT);
  for(i=0; i<count; i++) {
