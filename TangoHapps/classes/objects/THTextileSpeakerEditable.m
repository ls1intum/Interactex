...
#import "THTextileSpeakerEditable.h"
#import "THTextileSpeaker.h"
#import "THElementPinEditable.h"
#import "THElementPin.h"

@implementation THTextileSpeakerEditable
...
-(id) init{
    self = [super init];
    if(self){
        self.simulableObject = [[THTextileSpeaker alloc] init];
...
-(void) loadVibrationBoard{
    ...
    self.type = kHardwareTypeTextileSpeaker;
}
...
-(id)copyWithZone:(NSZone*) zone {
    THTextileSpeakerEditable * copy = [super copyWithZone:zone];
...
-(NSArray*)propertyControllers {
    ...
    [controllers addObject:[THTextileSpeakerEditable properties]];
...
-(void) updateToPinValue{
    ...
    if(lilypadPin.type == kPintypeDigital){
        ...
        THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
        textileSpeaker.sound = lilypadPin.value;
    }
}
...
-(void) adaptVibrationFrequency{
    ...
    if(self.sound > 0){
        _shakeAction = [self shakeActionWithFrequency:20];
...
-(BOOL) on{
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    return textileSpeaker.on;
}

-(void) setOn:(BOOL)on{
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    [textileSpeaker setOn:on];
}

-(NSInteger) sound{
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    return textileSpeaker.sound;
}

-(void) setSound:(NSInteger)sound{
    
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    textileSpeaker.sound = sound;
    if(self.on){
...
-(void) playSong:(NSInteger) sound{
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    return [textileSpeaker playSong: sound];
}

- (void)setSender:(BOOL) sender{
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    return [textileSpeaker setSender: sender];
}

-(void) setVolume:(float) volume{
    
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    return [textileSpeaker setVolume: volume];
}

-(void) setFrequency:(float) frequency{
    
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    return [textileSpeaker setFrequency: frequency];
}
...
- (void)turnOn{
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    [textileSpeaker turnOn];
}

- (void)turnOff{
    THTextileSpeaker * textileSpeaker = (THTextileSpeaker*)self.simulableObject;
    [textileSpeaker turnOff];
}

-(NSString*) description{
    return @"Radio Speaker";
}
