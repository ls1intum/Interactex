...
@interface THTextITConnectionController()

@property (retain, nonatomic) MCNearbyServiceBrowser * browser;

@end

@implementation THTextITConnectionController
...
- (id)init{
    if (self = [super init]) {
        ...
        self.browser = [[MCNearbyServiceBrowser alloc] initWithPeer:_localPeerID serviceType:kTextITConnectionServiceType];
...
- (void)session:(MCSession *)session didReceiveData:(NSData *)data fromPeer:(MCPeerID *)peerID {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if(receivingState == kReceivingStateNone){
            ...
            NSLog(@"received project name: %@",projectName);
            ...
            [NSKeyedUnarchiver setClass:[THCustomComponent class] forClassName:@"text_it.THCustomComponent"];
            THCustomComponent * project = [NSKeyedUnarchiver unarchiveObjectWithData:data];
            
            [self.delegate didFinishReceivingObject:project];
